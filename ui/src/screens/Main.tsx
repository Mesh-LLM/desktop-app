...
export default function Main({ onLeave }: MainProps) {
  ...
  const isPrivate = (status?.publication_state ?? 'private') === 'private'
    ...
    <div className="flex h-screen">
      ...
      <aside className="flex w-[270px] shrink-0 flex-col border-r border-edge bg-panel">
        ...
        <div className="mt-4 grow overflow-y-auto px-4">
          ...
          <div className="mt-5">
            <SectionLabel label="Models" count={models.length} />
            <ul className="mt-1 flex flex-col gap-2" data-testid="models-list">
              {models.map((m) => (
                <li key={m.id} className="text-[13px]">
                  ...
                  <div className="pl-4 text-[12px] text-ink-faint">ready</div>
                </li>
              ))}
              {models.length === 0 && (
                <li className="pl-4 text-[12px] text-ink-faint italic">none yet</li>
              )}
            </ul>
          </div>
        </div>
          ...
          <div className="relative">
            ...
            {settingsOpen && (
              ...
              >
                ...
                <hr className="my-3 border-edge" />
                <a
                  href={`http://localhost:${info?.console_port ?? 3131}`}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[13px] text-ink-muted hover:text-ink"
                >
