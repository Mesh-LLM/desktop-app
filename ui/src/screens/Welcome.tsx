...
interface WelcomeProps {
  ...
  onHost: () => void
  onOpenSettings: () => void
  activeMesh?: { name: string; isPublic: boolean }
  onReturnToChat?: () => void
}
...
export default function Welcome({
  ...
  return (
    <div className="flex h-screen flex-col">
      <AppNavigation
        current="home"
        connected={Boolean(activeMesh)}
        meshName={activeMesh?.name}
        isPublic={activeMesh?.isPublic}
        onHome={() => {}}
        onChat={() => onReturnToChat?.()}
        onSettings={onOpenSettings}
      />
      <div className="relative flex min-h-0 grow flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 flex w-full max-w-2xl flex-col items-center gap-8 px-8">
          ...
          >
            ...
            <p className="mt-2 max-w-lg text-sm leading-relaxed text-ink-muted">
              Thousands of machines of every size, pooling their compute and a whole variety of
              expert models — always on, no account, end-to-end encrypted. You can be part of it.
            </p>
